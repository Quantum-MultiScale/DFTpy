...
from dftpy.functional.kedf.lwt import LMGP, LMGPA, LMGPG, LWT
from dftpy.functional.kedf.mgp import MGP, MGP0, MGPA, MGPG
from dftpy.functional.kedf.sm import SM, SMStress
from dftpy.functional.kedf.tf import TF, TTF, ThomasFermiStress
from dftpy.functional.kedf.vw import vonWeizsackerStress, vW
...
class KEDF(AbstractFunctional):
    ...
    def compute(
        self, density, calcType={"E", "V"}, name=None, kedf=None, split=False, **kwargs
    ):
        if kedf:
            name = kedf
        if name is None:
...
class NLGGA(AbstractFunctional):
    ...
    def compute(self, density, calcType={"E", "V"}, **kwargs):
        ...
        if rhomax_w > nmax:
            sprint(
                '!WARN : some density large than rhomax',
                rhomax_w,
                nmax,
                comm=density.mp.comm,
                level=1,
            )
...
class MIXGGAS(AbstractFunctional):
    ...
    def compute(self, density, calcType={"E", "V"}, **kwargs):
        ...
        if 'V' in calcType:
            pot = (
                interpolate_f * func_stv.potential
                + interpolate_df * func_stv.energydensity
            )
            pot += (
                1 - interpolate_f
            ) * func_gga.potential - interpolate_df * func_gga.energydensity
            obj.potential = pot
    ...
    def _interp_lkt(self, rho, calcType={"E", "V"}, **kwargs):
        ...
        F, dFds2 = GGAFs(
            s, functional='LKT', calcType=calcType, gga_remove_vw=True, **kwargs
        )
