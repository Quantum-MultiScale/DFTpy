...
def _line_search_vector(func, alpha0, method='wolfe', func0=None, repeat=1, **kwargs):
    ...
    for _ in range(repeat):
        ...
        for i, a in enumerate(alpha):
            ...
            res = line_search(
                fobj, xs[i], alpha0=a, method=method, func0=func0s[i], **kwargs
            )
            nit += res[2]
            if res[0] is None:
                xs = None
...
def minimize(func, alpha0, method='powell', **kwargs):
    ...
    if method.lower() in ['powell']:
        res = sopt.minimize(
            ...
            alpha0,
            method=method,
            options={'direc': np.eye(np.asarray(alpha0).size) * alpha0},
        )
    else:
...
def Brent(func, alpha0=None, brack=(0.0, 1.0), tol=1e-8, full_output=1):
    ...
    alpha1, x1, _, i = sopt.brent(
        f, alpha0, brack=brack, tol=tol, full_output=full_output
    )
...
def interpolation_3d(arr, nr_new, interp="map"):
    ...
    if interp == "map":
        ...
        new_values = interpolate.griddata(
            points, values.ravel(), (X, Y, Z), method="linear"
        )
    return new_values
...
def restriction(arr, scheme="bilinear"):
    ...
    f[1 : nr[0] + 1, :, :] = 0.25 * (
        f[: nr[0], :, :] + 2.0 * f[1 : nr[0] + 1, :, :] + f[2:, :, :]
    )
    f[:, 1 : nr[0] + 1, :] = 0.25 * (
        f[:, : nr[0], :] + 2.0 * f[:, 1 : nr[0] + 1, :] + f[:, 2:, :]
    )
    f[:, :, 1 : nr[2] + 1] = 0.25 * (
        f[:, :, : nr[2]] + 2.0 * f[:, :, 1 : nr[2] + 1] + f[:, :, 2:]
    )
    nr2 = nr // 2 + 1
...
def prolongation(arr, scheme="bilinear"):
    ...
    # line
    new_values[1::2, ::2, ::2] = (
        f2[: nr[0], : nr[1], : nr[2]] + f2[1:, : nr[1], : nr[2]]
    )
    new_values[::2, 1::2, ::2] = (
        f2[: nr[0], : nr[1], : nr[2]] + f2[: nr[0], 1:, : nr[2]]
    )
    new_values[::2, ::2, 1::2] = (
        f2[: nr[0], : nr[1], : nr[2]] + f2[: nr[0], : nr[1], 1:]
    )
    # plane
...
def get_direction_CG(resA, dirA=None, method="CG-HS", **kwargs):
    ...
    if len(resA) == 1:
        ...
        method in ["CG-HS", "CG"] and len(dirA) > 0
    ):  # Works better than others in most tests
        beta = asum(resA[-1] * (resA[-1] - resA[-2])) / asum(
...
def gaussian(x, sigma=0.1, mu=0.0, dim=3, deriv=0):
    y = (
        1.0
        / (np.sqrt(2.0 * np.pi) * sigma) ** dim
        * np.exp(-0.5 * ((x - mu) / sigma) ** 2.0)
    )
    if deriv == 0:
        pass
    elif deriv == 1:
